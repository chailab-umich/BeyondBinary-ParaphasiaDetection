...
def compile_predictions_labels_awer(results_dict, labels_dict):
    ...
    for utt_id, label_aug_para in labels_dict.items():
        ...
        word_recognition_labels = [
            l.split("/")[0] for l in label_aug_para if l != "<eps>"
        ]
...
def extract_GPT_data_oracle(gpt_dir):
    ...
    for i in range(1, 13):
        ...
        (
            ...
            y_pred_fold,
            awer_df_fold,
        ) = compile_predictions_labels_awer(results, labels_dict)
        ...
        df_loc = pd.DataFrame(
            {
                ...
                "awer-disj": awer_df_fold["awer_disj-err"]
                / awer_df_fold["awer_disj-tot"],
                "awer-PD": awer_df_fold["awer_PD-err"]
                / awer_df_fold["awer_PD-tot"],
                "TD_bin": TD_list_bin,
...
def anova_test(df):
    ...
    for score in scores:
        ...
        agg_df = df_score.groupby(["uids", "model"], as_index=False)[
            score
        ].mean()
        # Perform the Repeated Measures ANOVA
        anova = AnovaRM(
            data=agg_df, depvar=score, subject="uids", within=["model"]
        )
        result = anova.fit()
...
if __name__ == "__main__":
    ...
    filtered_df = df.groupby("uids").filter(
        lambda x: all_models.issubset(x["model"].unique())
    )
