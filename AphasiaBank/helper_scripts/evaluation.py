...
def display_and_save_dfs(eval_dir,utt_df, fold_df, utt_stats_df, df_utt_stat_sig=None):
    ...
    with open(f"{results_dir}/Frid_metrics_multi.txt", 'w') as w:
        ...
        for k in ['wer','awer','awer_disj', 'awer_PD']:
            err = utt_df[f'{k}-err'].values.sum()/utt_df[f'{k}-tot'].values.sum()
            ...
            if line.startswith("P") and len(line.split()) == 14 and switch == 0:
                ...
                for word in words:
                    if "<eps>" in word:
                        ...
                        new_para_arr = [word.split("/")[0], f'[{word.split("/")[1].lower()}]']
...
def mtl_get_metrics(fold_dir,fold_num):
    ...
    return utt_stats_df, fold_stats_df, df_awer_stat_sig, list_list_ytrue, list_list_ypred
...
def ss_get_metrics(fold_dir, fold_num):
    ...
    noalign_ytrue, noalign_ypred, true_tag_list, pred_tag_list = ss_remove_paraphasia_tags(noalign_ytrue, noalign_ypred)
...
def para_eval(eval_dir, model_name):
    ...
    for i in range(1,13):
        ...
        if model_name == 'mtl':
            utt_stats_df, fold_stats_df, df_awer_stat_sig, list_list_ytrue, list_list_ypred = mtl_get_metrics(Fold_dir, i)
        elif model_name == 'single_seq':
            utt_stats_df, fold_stats_df, df_awer_stat_sig, list_list_ytrue, list_list_ypred = ss_get_metrics(Fold_dir, i)
    ...
    display_and_save_dfs(eval_dir, utt_df, fold_df, utt_stats_df, df_utt_stat_sig)
...
def compile_predictions_labels_awer(results_dict, labels_dict):
    ...
    AWER_dict = {
                'wer-err':[], 'wer-tot':[],
                'awer-err':[], 'awer-tot':[],
                 'awer_disj-err':[], 'awer_disj-tot':[],
                 'awer_PD-err':[], 'awer_PD-tot':[]}
    ...
    for utt_id, label_aug_para in labels_dict.items():
        ...
        AWER_dict['wer-err'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'])
        AWER_dict['wer-tot'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'] + measures['hits'])
        ...
        AWER_dict['awer-err'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'])
        AWER_dict['awer-tot'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'] + measures['hits'])
        ...
        AWER_dict['awer_disj-err'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'])
        AWER_dict['awer_disj-tot'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'] + measures['hits'])
        ...
        AWER_dict['awer_PD-err'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'])
        AWER_dict['awer_PD-tot'].append(measures['substitutions'] + measures['deletions'] + measures['insertions'] + measures['hits'])
...
def GPT_eval(gpt_dir,model_name):
    ...
    for i in range(1,13):
        ...
        fold_stats_df = pd.DataFrame({
            ...
            'awer_PD': [sum(AWER_dict['awer_PD-err'])/sum(AWER_dict['awer_PD-tot'])],
        })
