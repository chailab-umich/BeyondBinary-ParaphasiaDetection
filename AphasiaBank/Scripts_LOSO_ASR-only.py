...
if __name__ == "__main__":
    ...
    if FREEZE_ARCH:
        BASE_MODEL = (
            f"ISresults/ASR-only_Proto/frozen_S2S-hubert-Transformer-500"
        )
        EXP_DIR = (
            f"ISresults/ASR-only_Scripts/frozen_S2S-hubert-Transformer-500"
        )
    else:
    ...
    if TRAIN_FLAG:
        yaml_src = (
            "/home/mkperez/speechbrain/AphasiaBank/hparams/Scripts/ASR_base.yml"
        )
    ...
    if EVAL_FLAG:
        ...
        with open(f"{results_dir}/Frid_metrics_multi.txt", "w") as w:
            for k in ["wer"]:
                ...
                print(
                    f"{k}-fold: {df[f'{k}-fold'].mean()} {df[f'{k}-fold'].std()}"
                )
                w.write(
                    f"{k}-fold: {df[f'{k}-fold'].mean()} {df[f'{k}-fold'].std()}\n"
                )
