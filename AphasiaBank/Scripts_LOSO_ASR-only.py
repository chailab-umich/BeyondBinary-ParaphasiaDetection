...
if __name__ == "__main__":
    ...
    if FREEZE_ARCH:
        BASE_MODEL = f"ISresults/ASR-only_Proto/frozen_S2S-hubert-Transformer-500"
        EXP_DIR = f"ISresults/ASR-only_Scripts/frozen_S2S-hubert-Transformer-500"
    else:
        BASE_MODEL = f"ISresults/ASR-only_Proto/S2S-hubert-Transformer-500"
        EXP_DIR = f"ISresults/ASR-only_Scripts/S2S-hubert-Transformer-500"

    EXP_DIR = f"ISresults/ASR-only_Scripts/S2S-hubert-Transformer-500"


    if TRAIN_FLAG:
        yaml_src = "/home/mkperez/speechbrain/AphasiaBank/hparams/Scripts/ASR_base.yml"
        yaml_target = "/home/mkperez/speechbrain/AphasiaBank/hparams/Scripts/ASR_final.yml"
        start = time.time()
        ...
        while i <=12:
            ...
            cmd = ['python', '-m', 'torch.distributed.launch',
                   f'--master_port={str(port)}', 
                'train_ASR-only.py', f'{yaml_target}',
                '--distributed_launch', '--distributed_backend=nccl', '--find_unused_parameters']
