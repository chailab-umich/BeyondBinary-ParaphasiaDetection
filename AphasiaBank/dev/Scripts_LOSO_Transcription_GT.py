"""
Use GT model if available
Make backward compatible if GT_bool is toggled
"""
import os
import shutil
import subprocess
...
def compute_time_tolerant_scores(true_labels, predicted_labels, n=0):
    ...
    assert len(true_labels) == len(
        predicted_labels
    ), "Length of true_labels and predicted_labels must be the same"
    ...
    for utt_true, utt_pred in zip(true_labels, predicted_labels):
        ...
        for i, (true_label, predicted_label) in enumerate(
            zip(utt_true, utt_pred)
        ):
            neighborhood = utt_pred[
                max(i - n, 0) : min(i + n + 1, len(utt_pred))
            ]
...
def get_metrics(fold_dir, paraphasia):
    ...
    i = 0
    # remove eps
    noalign_ytrue, noalign_ypred = extract_word_level_paraphasias(
        wer_file, paraphasia
    )
    ...
    (
        ...
        true_tag_list,
        pred_tag_list,
    ) = remove_paraphasia_tags(noalign_ytrue, noalign_ypred)
    ...
    result_df = pd.DataFrame(
        {"wer-err": [wer_details["err"]], "wer-tot": [wer_details["tot"]],}
    )
...
def change_yaml(
    ...
    with open(yaml_target) as fin:
        ...
        filedata = filedata.replace("gt_PLACEHOLDER", f"{prepend_gt}")
...
if __name__ == "__main__":
    ...
    if FREEZE_ARCH:
        ...
        EXP_DIR = f"/home/mkperez/speechbrain/AphasiaBank/ISresults/full_FT_Transcription_Scripts/GT_S2S-hubert-Transformer-500"
    ...
    if TRAIN_FLAG:
        yaml_src = "/home/mkperez/speechbrain/AphasiaBank/hparams/dev/transcription_GT_base.yml"
        yaml_target = "/home/mkperez/speechbrain/AphasiaBank/hparams/dev/transcription_GT_final.yml"
        start = time.time()
    ...
    if EVAL_FLAG:
        ...
        for i in range(1, 13):
            ...
            result_df, list_list_ytrue, list_list_ypred = get_metrics(
                Fold_dir, PARAPHASIA_EVAL
            )
        ...
        zero_f1, zero_recall = compute_time_tolerant_scores(
            list_list_ytrue_aggregate, list_list_ypred_aggregate, n=0
        )
        one_f1, one_recall = compute_time_tolerant_scores(
            list_list_ytrue_aggregate, list_list_ypred_aggregate, n=1
        )
        two_f1, two_recall = compute_time_tolerant_scores(
            list_list_ytrue_aggregate, list_list_ypred_aggregate, n=2
        )
        ...
        with open(
            f"{results_dir}/Frid_metrics_{PARAPHASIA_EVAL}.txt", "w"
        ) as w:
            for k in ["wer"]:
