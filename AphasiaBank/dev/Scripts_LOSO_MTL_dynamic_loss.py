        ...
        for line in lines:
            ...
            if line.startswith("P") and len(line.split()) == 14 and switch == 0:
                ...
                measures = jiwer.compute_measures(
                    true_word_para_disjoint, pred_word_para_disjoint
                )
                wer_details["err"] += (
...
def output_json_statistical_significance(fold_dir, results_dir, spkr_flag):
    ...
    # TD
    TD_per_utt, TD_list = compute_temporal_distance(
        list_list_ytrue, list_list_ypred
    )
    ...
    if spkr_flag:
        ...
        results_dict[f"awer-disj"].append(
            awer_disjoint["err"] / awer_disjoint["tot"]
        )
...
def change_yaml(
    ...
    lr = 5.0e-3  # 1e-3 for frozen arch
    ...
    with open(yaml_target) as fin:
        ...
        filedata = filedata.replace(
            "dynamic_alpha_PLACEHOLDER", f"{dynamic_alpha}"
        )
...
if __name__ == "__main__":
    ...
    if FREEZE_ARCH:
        ...
        EXP_DIR = f"/home/mkperez/speechbrain/AphasiaBank/ISresults/full_FT_MTL_Scripts/dynamic-{DYNAMIC_ALPHA}_lr-0.005_S2S-hubert-Transformer-500"

    if TRAIN_FLAG:
        yaml_src = (
            "/home/mkperez/speechbrain/AphasiaBank/hparams/dev/MTL_base.yml"
        )
        yaml_target = (
            "/home/mkperez/speechbrain/AphasiaBank/hparams/dev/MTL_fold.yml"
        )
        start = time.time()
    ...
    if EVAL_FLAG:
        ...
        for i in range(1, 13):
            ...
            output_json_statistical_significance(
                Fold_dir, results_dir, SPKR_FLAG_STAT_SIG
            )
