'''
Use phone mappings and generate pseudoword using this
For Speechbrain E2E models
'''
import re
import hashlib
import itertools
...
def clean(text, utt, lexicon, oov_dict):
    ...
    print("preprocess: {}".format(text))
    text = __preprocess(text)
    print("post preprocess: {}".format(text))
    ...
    print("post partial omit: {}".format(text))
...
def __proc_special_forms(text, utt):
    ...
    for idx, w in zip(range(len(text)), text):
        if __is_word(w) and not w.startswith('<'):
            ...
            elif any([i in w for i in ['@q', '@a', '@d', '@u', '@wp']]):
    ...
    for w in text:
        if __is_word(w) and w not in lexicon:
            if common.is_ascii(w):
                ...
                if prons == None:
                    return ""
                word = phoneme_to_word(prons)
                print("prons: {}".format(prons))
                # exit()
...
def phoneme_to_word(phonemes):
    ...
    phone_str = phonemes[0]
    phoneme_arr = phone_str.split()

    # Mapping from CMU phonemes to approximate English characters
    phoneme_mapping = {
        'ae': 'a', 'aw':'a', 'dh': 'd', 'ng': 'n',
        'aa': 'a', 'ao': 'o', 'ah': 'a',
        'eh': 'e', 'er': 'er', 'g': 'g',
        ...
        'n': 'n', 'p': 'p', 's': 's',
        't': 't', 'v': 'v', 'w': 'w', 'z': 'z',
        'iy': 'i', 'uw': 'u', 'ow': 'o', 'oy':'oy',
        'y': 'y', 'ey': 'a', 'a':'a', 'ay': 'i',
    }

    # Convert phonemes to word using mapping
    word = ''.join([phoneme_mapping[phoneme] for phoneme in phoneme_arr if phoneme in phoneme_mapping])
...
def __finalize(text, utt):
    ...
    if len(text) == 0 or text[0] is None:
        return text
    return [w for w in text if __is_word(w)]
