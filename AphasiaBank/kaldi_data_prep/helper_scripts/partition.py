...
def main():
    ...
    for db in db2spk.keys():
        if db not in ["star", "UCL","MBA"]:
            io.log('Partitioning speakers for {}'.format(db))
            partitions[db] = random_partition(db2spk[db], args.nfolds, rand=rand)
    ...
    for fold in range(args.nfolds) + [-1]:
        ...
        for db in partitions.keys():
            io.log('...processing {}'.format(db))
            # Test set
            if fold != -1:
                for test_spk in partitions[db][fold]:
                    set_utts['test'].extend(spk2utt[test_spk])
                io.log('Added {} spks to test'.format(len(partitions[db][fold])))
            # Training and development set
            other_spks = []
            for other_fold in [f for f in range(args.nfolds) if f != fold]:
                other_spks.extend(partitions[db][other_fold])
